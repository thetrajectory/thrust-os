// services/videocx/publicCompanyService.jsx
...
export async function processPublicCompanyFilter(data, logCallback, progressCallback) {
    ...
    for (let j = 0; j < currentBatchSize; j++) {
      ...
      try {
        ...
        if (!orgId) {
          ...
          skippedCount++;
          processedData[index].relevanceTag = 'No Organization ID';
          continue;
        }
        ...
        if (supabaseAvailable) {
          const { data: dbData, error } = await supabase
            .from('orgs_db')
            .select('company_type, market_listed, updated_at, created_at')
  ...
  // Apply tagging for private companies
  processedData.forEach(row => {
    if (row.companyType === 'Private' && !row.relevanceTag) {
      row.relevanceTag = 'Private Company';
    }
  });
